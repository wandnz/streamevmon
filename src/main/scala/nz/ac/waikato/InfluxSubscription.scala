          ...
          with Logging
{
  ...
  def destinationStrings: Seq[String] =
    Seq(s"$protocol://$listenAddress:$listenPort")
  ...
  override def run(): Unit =
  {
    ...
    while (shouldKeepRunning)
    {
      ...
      {
        case ex: SocketTimeoutException => ex
      }
    }
      ...
      else
      {
        dropSubscription(influxDB).flatMap(dropResult =>
        {
          if (dropResult.isRight)
          {
            addSubscription(influxDB).flatMap(updateResult =>
            {
              ...
              else
              {
                Future(
                  logger.error(
                    s"Failed to re-add subscription: ${updateResult.left.get}"
                  )
                ).flatMap(_ => Future(System.exit(1)))
              }
            })
          }
          else
          {
            Future(
              logger
                .error(s"Failed to drop subscription: ${dropResult.left.get}")
            ).flatMap(_ => Future(System.exit(1)))
          }
        })
      }
  ...
  def addSubscription(
    influxDB: AhcManagementClient
  ): Future[ErrorOr[ResponseCode]] =
  {
  ...
  {
    influxDB.dropSubscription(subscriptionName, dbName, rpName)
  }
}
...
object InfluxSubscription
{
  ...
  def main(args: Array[String]): Unit =
  {
    ...
    println("Started server. Waiting 10s...")
    Thread.sleep(10000)
